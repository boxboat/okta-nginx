...
ensure_file () {
    if [ ! -f "$1" ]; then
        cp "/etc/nginx/templates/blank.conf" "$1"
    fi
}

# clear generated files from previous run
find /etc/nginx/conf.d/ /etc/nginx/includes -type f \
    | xargs grep "# okta-nginx generated" -sl \
    | xargs rm -f
...
while : ; do
    ...
    ensure_file "/etc/nginx/includes/default-server${SERVER_SUFFIX}.conf"
    ensure_file "/etc/nginx/includes/proxy-pass-protected${SERVER_SUFFIX}.conf"
    ensure_file "/etc/nginx/includes/proxy-pass-unprotected${SERVER_SUFFIX}.conf"
...
ensure_file /etc/nginx/includes/refresh-js.conf
